...
FunctionSummary CFEPAnalyzer<FunctionOracle>::analyze(
  ...
  // Change the maximum value that MemorySSA uses to take into account stores
  // and phis.
...
CFEPAnalyzer<FunctionOracle>::integrateFunctionCallee(llvm::BasicBlock *BB) {
  ...
  case FunctionTypeValue::Regular: {
    ...
    // Extract MetaAddress of last `newpc`. This is passed later as argument to
    // call markers.
    auto *NewPCCall = reverseNewPCTraversal(Br);
    revng_assert(NewPCCall != nullptr);
    auto NewPCStruct = GCBI::getPCFromNewPC(NewPCCall);
    ...
    SmallVector<Value *, 1> Args = { NewPCStruct.toConstant(
...
bool StackAnalysis::runOnModule(Module &M) {
  ...
  // Initialize hook markers for subsequent ABI analysis.
